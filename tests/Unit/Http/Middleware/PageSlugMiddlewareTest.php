...
test('filterMiddlewareGroups strips web and api groups from cms json', function (): void {
    $middleware = new PageSlugMiddleware();

    /** @var array<int, string> $filtered */
    $filtered = invokeProtected($middleware, 'filterMiddlewareGroups', [['web', 'auth', 'api']]);

    Assert::assertSame(['auth'], $filtered);
});
