...
class PageSlugMiddleware
{
    ...
    public function handle(Request $request, \Closure $next): Response
    {
        ...
        $middlewares = $this->filterMiddlewareGroups($middlewares);
    ...
    protected function executeMiddlewareChain(Request $request, array $middlewares, \Closure $finalNext): Response
    {
        ...
        if (! $this->isResolvableMiddleware($middlewareClass)) {
    ...
    /**
     * Skip middleware groups already applied by Folio (web, api).
     *
     * @param array<int, string> $middlewares
     *
     * @return array<int, string>
     */
    protected function filterMiddlewareGroups(array $middlewares): array
    {
        return array_values(array_filter(
            ...
            static function (string $middleware): bool {
                ...
                return ! \in_array($name, ['web', 'api'], true);
            },
        ));
    }

    protected function isResolvableMiddleware(string $middleware): bool
    {
        if (\in_array($middleware, ['web', 'api'], true)) {
            return false;
        }
