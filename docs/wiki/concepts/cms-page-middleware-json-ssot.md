...
## Perché NON mettere `web`

`PageSlugMiddleware` **ignora** `web` e `api` nel JSON (gruppi già sullo stack Folio). Se presenti per errore, non causano più 500.

Alias validi nel JSON: `auth`, `verified`, classi FQCN, parametri `throttle:60,1`.
